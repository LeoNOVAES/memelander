...
const repository = {
  store: async (meme: MemeDocument): Promise<void> => {
    try {
  ...
  upsert: async (meme: MemeDocument): Promise<void> => {
    try {
  ...
  findAll: async (query: Record<string, unknown>): Promise<MemeDocument[]> => {
    try {
  ...
  findById: async (id: string): Promise<MemeDocument | null> => {
    try {
  ...
  findAllPaginated: async (page = 1, limit = 25): Promise<MemeDocument[]> => {
    try {
  ...
  random: async (size: number): Promise<MemeDocument[]> => {
